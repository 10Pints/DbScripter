...
namespace RSS.Test
{
   ...
   public class ParamsTests : UnitTestBase
   {
      ...
      public void ParseArgsWhenAllArgsSupplieThenPopCorrectlyGreenTest()
      {
         ...
         // Call the tested fn
         //-----------------------------------------------------------------------
         Assert.IsTrue(Params.ParseArgs( args, out Params p, out string msg), msg);
         ...
         Assert.IsNotNull(p.ScriptPath);
         Assert.IsNotNull(p.ScriptUseDb);
         Assert.IsNotNull(p.Server);
         Assert.IsNotNull(p.TargetChildTypes);
         ...
         Assert.AreEqual(Common.LogLevel.Trace  , p.LogLevel);                                        //"-log_level",    "Trace"
      ...
      public void ParseValidArgsTestNegative()
      {
         ...
         // run test
         Assert.IsTrue(Params.ParseArgs( args, out p, out msg), msg);
      ...
      public void ParseArgs_S_i_d_tg_ts_tru_rs_dbo_test_in_curlies_E_Test()
      {
         ...
         Assert.AreEqual( 5, p.TargetChildTypes.Count          , "Expected 5 types");
      ...
      public void ParseArgs_HandleArgError_Test()
      {
         ...
         // Run the Init() rtn
         Assert.IsFalse(Params.ParseArgs( args, out Params p, out string msg), msg);
         // Checkout the msg
         // chk mn err msg
...
";
      ...
      {
         var p = new Params();
         Assert.IsFalse(string.IsNullOrEmpty(p.Timestamp));
      }
...
*/
      ...
      public void Overlapping2Test()
      {
         ...
         Params P21_overlap_ovrwrt_exp = new(
            ...
            ,svrNm      : @"FRED"                 // was DevI9\SQLEXPRESS
