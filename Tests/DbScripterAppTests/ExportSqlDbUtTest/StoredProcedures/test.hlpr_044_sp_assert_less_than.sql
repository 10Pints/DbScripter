...
-- Description:      test helper rtn for the sp_assert_less_than rtn being tested
-- Tested rtn desc:
...
--    @fn_     NVARCHAR(60),
--========================================================================================
CREATE PROCEDURE [test].[hlpr_044_sp_assert_less_than]
   @a       SQL_VARIANT,
   @b       SQL_VARIANT,
   @msg     NVARCHAR(200),
   @msg2    NVARCHAR(200),
...
BEGIN
   DECLARE
      @fn                 NVARCHAR(35)   = N'hlpr_044_sp_assert_less_than'
...
---- RUN tested rtn:
   ...
   BEGIN
      BEGIN TRY
         ...
         EXEC sp_assert_less_than @a,@b,@msg,@msg2,@msg3,@msg4,@msg5
         ,@msg6,@msg7,@msg8,@msg9,@msg10,@msg11,@msg12,@msg13,@msg14
         ,@msg15,@msg16,@msg17,@msg18,@msg19,@msg20,@ex_num,@state;
