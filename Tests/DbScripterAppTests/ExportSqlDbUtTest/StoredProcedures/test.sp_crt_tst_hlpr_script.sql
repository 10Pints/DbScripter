...
BEGIN
   DECLARE 
    @fn                 NVARCHAR(35)   = 'sp_crt_tst_hlpr_script'
   ...
   ,@rtn_ty_code        NVARCHAR(25)
   ,@rtn_ty_nm          NVARCHAR(25)
   ,@rtn_nm             NVARCHAR(100)
   ,@tstd_rtn_call      NVARCHAR(250)
   ,@params             NVARCHAR(MAX)  = '/*params: <TBD>*/'
   ,@hlpr_params        NVARCHAR(MAX)  = '/*hlpr params: <TBD>*/'
   ,@max_param_len      INT            = -1
   ,@n                  INT
   ,@act_@qrn           NVARCHAR(100)
   ,@row_cnt            INT
   ,@act_tst_rtn_num    INT
   ,@error_num          INT
