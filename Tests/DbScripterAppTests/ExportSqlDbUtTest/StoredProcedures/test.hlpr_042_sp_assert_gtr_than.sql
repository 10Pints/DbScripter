...
-- Description:      test helper rtn for the sp_assert_gtr_than rtn being tested
-- Tested rtn desc:
...
--    @fn_     NVARCHAR(60),
--========================================================================================
CREATE PROCEDURE [test].[hlpr_042_sp_assert_gtr_than]
...
BEGIN
   DECLARE
      @fn                 NVARCHAR(35)   = N'hlpr_042_sp_assert_gtr_than'
   EXEC sp_log 1, @fn, '01: starting, @subtest: ', @tst_num;
---- SETUP:
   -- <TBD>
   ---- RUN tested rtn:
   ...
   BEGIN
      BEGIN TRY
         ...
         EXEC  sp_assert_gtr_than @a,@b,@msg,@msg2,@msg3,@msg4,@msg5,@msg6
               ,@msg7,@msg8,@msg9,@msg10,@msg11,@msg12,@msg13,@msg14,@msg15
               ,@msg16,@msg17,@msg18,@msg19,@msg20,@ex_num,@state;
...
---- CLEANUP:
   -- <TBD>
   EXEC sp_log 1, @fn, '99: leaving, ',@tst_num, ' PASSED';
END
/*
   EXEC tSQLt.RunAll;
