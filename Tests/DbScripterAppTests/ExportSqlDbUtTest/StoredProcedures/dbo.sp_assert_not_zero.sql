...
BEGIN
   DECLARE
      @fn                 NVARCHAR(35)   = N'sp_assert_not_zero'
   IF @ex_num IS NULL SET @ex_num = 50001; -- default
   EXEC sp_log 1, @fn, '000 starting';
   IF (@val = 0)
   BEGIN
      DECLARE @msg NVARCHAR(MAX)
      SET @msg = CONCAT('sp_assert_not_zero failed, value: ', @val, ' ', @msg1);
      EXEC sp_log 4, @fn, 'value is zero - raising exception ', @ex_num;
