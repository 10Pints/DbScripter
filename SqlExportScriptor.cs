...
namespace SI.Software.Databases.SQL
{
    ...
    /// </summary>
    public class SqlExportScriptor
    {
        ...
        public SqlExportScriptor(string databaseName = null, DbOpType opType = DbOpType.Undefined, string writerFilePath = null, string staticDataTables = null)
        {
            if (!string.IsNullOrEmpty(databaseName))
                Init(databaseName, opType, writerFilePath, staticDataTables);
        }
        ...
        /// <summary>
        /// Initialise state, deletes the writerFilePath file if it exists
        /// Only completes the initialisation if the paremteres are all specified
        /// </summary>
        /// <param name="databaseName"></param>
        /// <param name="opType"></param>
        /// <param name="writerFilePath"></param>
        ...
        private bool Init( string databaseName, DbOpType opType, string writerFilePath, string staticDataTables = null)
        {
            Status = false;
            ...
            try
            {
                ...
                // NOTE: can continue if not all initalised so long as the final init is performed before any write op
        ...
        private bool IsValid( )
        {
            ...
            do
            {
                ...
                // Lastly if here then all chks have passed
                isValid = true;
        ...
        /// <summary>
        /// Initialises file writer if writerFilePath is a valid path
        /// calls IsValid at end
        /// </summary>
        /// <param name="writerFilePath"></param>
        /// <returns>Result of the the validity check</returns>
        private bool InitWriter(string writerFilePath)
        {
            if (!string.IsNullOrEmpty(writerFilePath))
            {

                var fs = new FileStream(writerFilePath, FileMode.CreateNew);
        ...
        /// <summary>
        /// Initialises server connectio - default database: databaseName
        /// </summary>
        /// <param name="databaseName"></param>
        ...
        protected string ExportData(bool isStaticData, string exportFilePath)
        {
            ...
            Utils.Assertion(InitWriter(exportFilePath), "Script exporter not initialised");
        ...
        protected string ExportProceduresAndFunctions()
        {
            var sb = new StringBuilder();
            ExportFunctions(sb);
            ExportStoredProcedures(sb);
        ...
        protected void ExportStoredProcedures(StringBuilder sb)
        {
            LogUtils.LogS();
            Utils.Assertion<ConfigurationException>(Database != null, "ExportStoredProcedures(): Null database");
