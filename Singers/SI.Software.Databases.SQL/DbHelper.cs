...
namespace SI.Software.Databases.SQL
{
    ...
    /// See also DbContextExtensions class for EF support and DbTestHelper for test database support
    /// 
    /// It contains the following methods and support:
    /// Standard database error message strings
    /// Standard script file paths
    /// Standard App config database related key names
    /// Standard substitution tags like DB_NAME that are to be substituted in SQL string with the real name before the SQL is executed
    /// 
    ...
    /// 
    /// Public methods:
    /// Constructor                         Registers the Logger and gets an instance of the logger and set the app config  EfConnectionStringConfigKey - used to determine which connection string to use for the EF model
    ...
    /// DatabaseExists                      Checks the database exists - returns boolean
    ...
    /// GetAndSubstituteEfConnectionString  Returns the Entity Framework connection string substituted with the database name, server, instance
    ...
    /// GetConnectionStringBuilderFromConfig Creates an SqlConnectionStringBuilder from the application configuration and optional parameters
    /// GetDatabaseNameFromConfig           Gets the database name from configuration app settings section
    ...
    /// ProcessScript(db name,filePath,type map) Runs a SQL script file against the database, first replacing any supplied tag with their associated values
    ...
    public class DbHelper
    {
        ...
        // Used to ensure we dont delete the configuration database inadvertently
        ...
        /// get POSTCONDITION: returns non empty string if initially null sources from AppConfig settings
        /// key="DatabaseRootDir"
        /// </summary>
        public static string DatabaseRootDir
        {
            get
            {
                if (string.IsNullOrEmpty(_databaseRootDir))
                    DatabaseRootDir =  GetDatabaseRootDirFromConfig(); // This will call the set accessor and its validation
            ...
            set
            {
                ...
                if (!Directory.Exists(_databaseRootDir))
                {
                    var di = Directory.CreateDirectory(_databaseRootDir);
