...
BEGIN
DECLARE
    @fnThis    VARCHAR(35)   = N'sp_assert_not_empty'
   ,@msg       VARCHAR(MAX)
 ;

   EXEC sp_log @log_level, @fnThis, '000 starting @val: [',@val,']';
   ...
   BEGIN
      ...
      -- ASSERTION OK
      ----------------------------------------------------
      EXEC sp_log @log_level, @fnThis, '010: OK, @val:[',@val, '] is not empty';
      RETURN 0;
   END
   ...
   -- ASSERTION ERROR
   ----------------------------------------------------
   EXEC sp_log 4, @fnThis, '002: raising assert'
   SET @msg = CONCAT(@msg1, ', should not be empty ');
