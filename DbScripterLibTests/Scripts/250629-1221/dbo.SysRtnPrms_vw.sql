...
SELECT
   ...
   ,IIF( TYPE_NAME(system_type_id) IN ('VARCHAR', 'VARCHAR', 'NTEXT')
   ...
   ,iif
    ...
    , IIF(TYPE_NAME(system_type_id) IN ('VARCHAR', 'VARCHAR', 'NCHAR','CHAR','NTEXT'), 1, 0) AS is_chr_ty
   ,is_output
   ,is_nullable
   ,has_default_value
   ,default_value
...
/*
SELECT * FROM SysRtnPrms_vw WHERE rtn_nm = 'sp__main_import';
