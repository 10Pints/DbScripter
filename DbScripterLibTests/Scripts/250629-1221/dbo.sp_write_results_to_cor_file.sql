...
BEGIN
   DECLARE
    @fn              VARCHAR(35)   = N'WRITE_RSLTS_TO_COR_FILE'
   ,@sql             VARCHAR(MAX)
   ,@cnt             INT
...
';
   ...
   IF CHARINDEX('\', @cor_file) = 0
      SET @cor_file_path = CONCAT(dbo.fnGetImportRoot(), '\', @cor_file);
   ELSE
   BEGIN
      SET @cor_file_path = @cor_file;
      SET @cor_file = dbo.fnGetFileNameFromPath(@cor_file, 1); -- if @cor_file was spec'd as full path - set it to be the file only
   END

   EXEC sp_log 1, @fn,'010: updated params:
@cor_file:     [',@cor_file,      ']
...
';
    ...
    BEGIN TRY
      ...
      -- Processing
      -----------------------------------------------------------------------------------------------------------
      EXEC sp_log 2, @fn,'020: processing';
      SELECT @cnt = COUNT(*) FROM ImportCorrections
      EXEC sp_log 2, @fn,'030: updating Excel cor file, ImportCorrections has ', @cnt, ' rows    ';
      EXEC sp_executesql @sql, N'@cnt INT OUT', @cnt OUT;
      EXEC sp_log 2, @fn,'040: Excel cor file JOIN ImportCorrections has ', @cnt, ' common rows    ';
...
JOIN ImportCorrections ic ON xl.id=ic.id'
      ...
      -- Processing complete
      -----------------------------------------------------------------------------------------------------------
      EXEC sp_log 2, @fn, '050: completed processing;'
   END TRY
   BEGIN CATCH
      EXEC sp_log 4, @fn, '060: sql:',@sql;
