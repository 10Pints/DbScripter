...
BEGIN
   DECLARE 
       @fn                       VARCHAR(35)  = N'INVESTIGATE S2 CROPS'
      ,@sql                      NVARCHAR(MAX)
...
@case_sensitve    : [', @case_sensitve    , ']';
   ...
   BEGIN TRY
      ...
      SELECT @len1 = MAX(dbo.fnLen(crops))
      FROM
      (
      ...
      SELECT @len2 = MAX(dbo.fnLen(crops))
      FROM
      (
      SELECT DISTINCT crops
         FROM staging2
      ...
      -- agg ids
      SET @sql = dbo.fnGetIdsInTablesForCriteriaSql('staging1', 'id', 'staging2', 'id', @where_clause);
      ...
      BEGIN
         ...
         SET @sql = CONCAT('SELECT * from Crop WHERE crop_nm LIKE ''%', dbo.fnTrim2(@where_subclause1, '%'), '%''');
         EXEC(@sql);
      END
      ELSE
      BEGIN
