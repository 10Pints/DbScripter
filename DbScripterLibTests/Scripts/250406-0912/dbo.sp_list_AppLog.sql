...
BEGIN
   DECLARE
    @fn           VARCHAR(30)  = N'sp_list_AppLog'
   ,@sql          NVARCHAR(MAX)
   ,@fnFltrCls    VARCHAR(MAX)

   IF @asc IS NULL SET @asc = 1; -- DEFAULT = ASC
   SELECT @fnFltrCls = CONCAT('''', string_agg(dbo.fnTrim(value), ''','''), '''') from string_split(@fnFilter, ',');
...
,'        id
      ...
      , iif(@fnFilter    IS NULL, '', CONCAT(NCHAR(13), ' rtn IN (' , @fnFltrCls,')'))
   ...
   PRINT @sql;
   EXEC sp_executesql @sql, N'@fnFilter VARCHAR(50), @msgFilter VARCHAR(128), @minIdFilter INT, @maxIdFilter INT, @levelFilter INT'
...
EXEC sp_list_AppLog 'MN_IMPORT,POP STG TBLS', @minIdFilter = 2000, @asc=1   -- LIST the main import rtn, asc 
EXEC sp_list_AppLog 'MN_IMPORT,POP STG TBLS', @maxIdFilter = 2000, @asc=1   -- LIST the main import rtn, asc 
*/
