...
(
   fk_nm             VARCHAR(60),
   foreign_table_nm  VARCHAR(60),
   primary_tbl_nm    VARCHAR(60),
   fk_col_nm         VARCHAR(60),
   pk_col_nm         VARCHAR(60),
   ordinal           INT,
...
BEGIN
   ...
   FROM fKeys_vw
   WHERE
       (foreign_table_nm= @tbl_nm OR @tbl_nm IS NULL) OR (primary_tbl_nm = @tbl_nm)
...
SELECT * FROM dbo.fnListFkeys( 'Pathogen', NULL);                                 -- Should list all FKs in database
SELECT * FROM dbo.fnListFkeys( 'CropPathogen', NULL);                                 -- Should list all FKs in database
