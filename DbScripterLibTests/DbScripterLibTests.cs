...
{
   [TestClass]
   public class DbScripterLibTests : ScriptableUnitTestBase
   {
      //#region tests
      /// <summary>
      /// Bugs:
      ...
      "\"FilePath\": \"D:\\Dev\\DbScripter\\DbScripterApp\\AppSettings.json\"," +
      "\"Server\": \"DEVI9\"," +
      "\"Instance\": \"\"," +
      "\"Database\": \"Farming_dev\"," +
      "\"Script Dir\": \"D:\\Dev\\DbScripter\\Scripts\\Staging\"," +
      "\"Script File\": \"Farming.sql\"," +
      "\"CreateMode\": \"Alter\"," + 
      "\"RequiredSchemas\": \"dbo\"," +
      "\"RequiredTypes\": \"Schemas,Tables,Procedures,Functions,Views\"," +
      "\"AddTimestamp\": \"true\"," +
      "\"ScriptUseDb\": \"true\"," +
      "\"DisplayScript\": \"true\"," +
      "\"DisplayLog\": \"true\"," +
   ...
   "}";
         ...
         */
         Params p = new Params();
         // *** load from string config
         CreateConfigFromJsonString(json);
         Assert.IsTrue(sc.Init(p, out msg), msg);
         LogL();
      }

      public static void CreateConfigFromJsonString(string json)
      {
         var stream = new MemoryStream();
         var writer = new StreamWriter(stream);

         // Write the json string to the memory stream
         writer.Write(json);
         writer.Flush();
         stream.Position = 0;

         // Load the configuration from the memory stream
      ...
      public void ExportSchemas_Alter_2_cvdT1_dbo_tst_Test()
      {
         ...
         /*CovidBaseParams:   serverName:   @"DevI9\SQLEXPRESS"
      ...
      public void InitTableExportTest()
      {
         ...
         Params p = Params.PopParams(
             prms       : CovidBaseParams
            ,name         : "Fred"
            ,scriptFile  : @"C:\temp\InitTableExportTest.sql"
            ,createMode         : CreateModeEnum.Create
...
//         Assert.IsFalse(so?.ScriptForCreateOrAlter ?? true, "ScriptForCreateOrAlter");
      ...
      public void InitTableExportTestEx()
      {
         ...
         Params p = Params.PopParams(
            ...
            ,instanceName     : "SQLEXPRESS"
            ,databaseName       : "Covid_T1"
            ,scriptFile  : @"C:\temp\InitTableExportTestEx.sql"
            ,createMode         : CreateModeEnum.Alter
            ,requiredSchemas        : "tEst,tSqlt"
            ,requiredTypes        : "table"
          );
      ...
      public void InitScriptingOptionsWhenTAlterAndTableThenExpFalseTest()
      {
         ...
         Params p = Params.PopParams(
            ...
            ,name         : "Jog=hn"
            ,databaseName       : null
            ,scriptFile  : @"C:\temp\T011_ExportSchemaScriptTest.sql"
            ,createMode         : CreateModeEnum.Alter
            ,requiredSchemas        : "tEst,tSqlt"
      ...
      public void MapTypeToSqlTypeTest()
      {
         ...
         Params p = Params.PopParams( 
             prms       : CovidBaseParams
            ,scriptFile  : @"C:\temp\MapTypeToSqlTypeTest.sql"
            ,createMode         : CreateModeEnum.Create
            ,requiredSchemas        : "tEst,tSqlt"
           );
      ...
      public void MapTypeToSqlTypeTestUnknownTypeTest()
      {
         ...
         { 
            Params p = Params.PopParams( 
             prms       : CovidBaseParams
            ,scriptFile  : @"C:\temp\MapTypeToSqlTypeTestUnknownTypeTest.sql"
            ,createMode         : CreateModeEnum.Create
            ,requiredSchemas        : "tEst,tSqlt"
            );
      ...
      public void ExportSchemas_spaces_Test()
      {
         ...
         Params p = Params.PopParams(
            ...
            , scriptFile: ScriptFile
            , createMode: CreateModeEnum.Create
            , requiredSchemas: "dbo"            // should handle more than 1 schema and crappy formatting
            , displayScript: true
            );
