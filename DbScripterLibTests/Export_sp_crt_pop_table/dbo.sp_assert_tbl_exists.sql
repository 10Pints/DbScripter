...
CREATE PROCEDURE [dbo].[sp_assert_tbl_exists]
   ...
   ,@exp_exists      BIT            = 1
   ,@msg_0           VARCHAR(8000)  = NULL
   ,@msg_1           VARCHAR(8000)  = NULL
   ,@msg_2           VARCHAR(8000)  = NULL
   ,@msg_3           VARCHAR(8000)  = NULL
   ,@msg_4           VARCHAR(8000)  = NULL
AS
BEGIN
   DECLARE
   ...
   BEGIN -- failed test
      ...
      EXEC sp_raise_exception 50001, @msg, @msg_0, @msg_1, @msg_2, @msg_3, @msg_4, @fn=@fn;
   END
